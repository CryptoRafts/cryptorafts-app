...
export default function FounderKYCWizard() {
  const { completeKYC, analyzeKYC } = useFounderAuth();
  ...
  const handleSubmit = async () => {
    if (!documents.idFront || !documents.idBack || !documents.proofOfAddress || !documents.selfie) {
    ...
    try {
      ...
      const kycData = {
        status: 'pending',
        documents: {
          idFront: `kyc/${Date.now()}-id-front.jpg`,
          idBack: `kyc/${Date.now()}-id-back.jpg`,
          proofOfAddress: `kyc/${Date.now()}-proof-of-address.jpg`,
          selfie: `kyc/${Date.now()}-selfie.jpg`
        },
        submittedAt: Date.now()
      };
      
      // Analyze with RaftAI
      const result = await analyzeKYC(kycData);
      
      setAnalysisResult(result);
      
      // Complete KYC with result
      await completeKYC({
        ...kycData,
        ...result,
        updatedAt: Date.now()
      ...
      if (result.status === 'approved') {
        ...
        // Stay on page to show rejection reasons
      } else {
        // Pending - show pending message
      }
      
    } catch (error: any) {
      setError(error.message || 'KYC verification failed');
    } finally {
        ...
        <div className="text-center space-y-6">
          ...
          {analysisResult.status === 'pending' && (
            <div className="space-y-4">
              ...
              <h3 className="text-2xl font-bold text-yellow-400">KYC Pending</h3>
              <p className="text-gray-300">
    ...
    if (isAnalyzing) {
      return (
        <div className="text-center space-y-6">
          ...
          <p className="text-gray-300">
            RaftAI is verifying your identity. This usually takes 5-10 seconds.
          </p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 pt-20">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 p-8 shadow-2xl">
          ...
          {/* Submit Button */}
          {allStepsCompleted && !isAnalyzing && !analysisResult && (
            <div className="mt-8 text-center">
              <button
                onClick={handleSubmit}
                className="px-8 py-4 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-semibold rounded-xl transition-all duration-200 shadow-lg hover:shadow-green-500/25"
              >
                <span className="flex items-center justify-center space-x-2">
