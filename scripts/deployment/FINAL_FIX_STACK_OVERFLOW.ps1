# ============================================
# FINAL FIX: STACK OVERFLOW & FIREBASE INIT
# ============================================
...
Write-Host ""
Write-Host "FIXING STACK OVERFLOW & FIREBASE INITIALIZATION" -ForegroundColor Cyan
...
$tempFile = "$env:TEMP\fix_stack_overflow.sh"
...
scp $tempFile "${vpsUser}@${vpsIP}:/tmp/fix_stack_overflow.sh" 2>&1 | Out-Null
ssh "$vpsUser@$vpsIP" "chmod +x /tmp/fix_stack_overflow.sh && bash /tmp/fix_stack_overflow.sh"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "  ✅ SUCCESS: STACK OVERFLOW FIXED!" -ForegroundColor Green
    Write-Host "═══════════════════════════════════════════════════════" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "  ✅ Stack overflow error fixed (infinite recursion)" -ForegroundColor Green
    Write-Host "  ✅ Firebase initialization improved" -ForegroundColor Green
    Write-Host "  ✅ Database initialization with retry logic" -ForegroundColor Green
    Write-Host "  ✅ Auth null check fixed" -ForegroundColor Green
    Write-Host ""
