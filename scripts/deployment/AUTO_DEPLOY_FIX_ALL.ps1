# ============================================
# CRYPTORAFTS - AUTOMATED DEPLOYMENT WITH REAL-TIME ERROR FIXING
# ============================================
# This script deploys everything and auto-fixes errors in real-time
...
Write-Host "📋 Starting automated deployment with real-time error fixing..." -ForegroundColor Yellow
Write-Host ""

# Complete deployment command with error handling and auto-fix
$deployCommand = @"
cd $appPath 2>/dev/null || mkdir -p $appPath && cd $appPath && 
echo "🚀 AUTOMATED DEPLOYMENT WITH AUTO-FIX - www.cryptorafts.com" && 
echo "========================================" && 
echo "" && 
  ...
  case \$error_type in
    "missing_files")
      ...
      if [ ! -f package.json ]; then
        echo "❌ package.json missing - cannot continue without it"
        exit 1
      fi
      ;;
    "npm_install_failed")
    ...
    "pm2_not_running")
      echo "📋 Restarting PM2..."
      pm2 start ecosystem.config.js || pm2 restart cryptorafts
      ;;
    "nginx_not_running")
...
ENVEOF
chmod 600 .env.local && 
echo "✅ .env.local created/updated" && echo "" && 

# Step 3: Stop PM2 and clean (with error handling)
...
echo "✅ Node.js version: \$NODE_VERSION" && echo "" && 

# Step 5: Install dependencies (with error handling and auto-fix)
...
# Step 6: Rebuild application (with error handling and auto-fix)
...
while [ \$BUILD_ATTEMPTS -lt \$MAX_BUILD_ATTEMPTS ]; do
  ...
  if [ \$BUILD_STATUS -eq 0 ]; then
    ...
    if grep -q "Cannot find module" build.log; then
      ...
      grep -o "Cannot find module '[^']*'" build.log | sed "s/Cannot find module '//;s/'//" | xargs -I {} npm install {} --legacy-peer-deps || true
    fi
...
# Step 9: Start PM2 (with error handling and auto-fix)
...
# Step 12: Final verification with error checking
...
echo "✅ AUTOMATED DEPLOYMENT WITH AUTO-FIX COMPLETE!"
echo "========================================"
echo ""
echo "📊 Summary:"
...
Write-Host "🔗 Executing automated deployment on VPS..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "`n💡 Alternative: Copy the command from AUTO_DEPLOY_SSH.txt and run it manually" -ForegroundColor Yellow
}

Write-Host "`n================================================" -ForegroundColor Cyan
Write-Host "✅ AUTOMATED DEPLOYMENT SCRIPT READY!" -ForegroundColor Green
Write-Host "================================================`n" -ForegroundColor Cyan
