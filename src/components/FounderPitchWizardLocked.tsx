...
type DocumentState = Record<'pitchDeck' | 'whitepaper' | 'tokenModel' | 'audits', File | null>;
type DocumentUrlState = Record<'pitchDeck' | 'whitepaper' | 'tokenModel' | 'audits', string>;

const createInitialPitchData = (): PitchData => ({
  projectName: '',
  sector: '',
  chain: '',
  stage: '',
...
const createInitialDocumentUrlState = (): DocumentUrlState => ({
  pitchDeck: '',
  whitepaper: '',
  tokenModel: '',
  audits: '',
...
export default function FounderPitchWizardLocked() {
  ...
  const [formData, setFormData] = useState<PitchData>(createInitialPitchData());

  const [documents, setDocuments] = useState<DocumentState>(createInitialDocumentState());

  const [documentUrls, setDocumentUrls] = useState<DocumentUrlState>(createInitialDocumentUrlState());
  ...
  const validateStep = (stepIndex: number): boolean => {
    ...
    switch (step.id) {
      case 'basics':
        return !!formData.projectName && !!formData.sector && !!formData.chain && !!formData.stage;
      ...
      case 'documents':
        return !!documents.pitchDeck; // Only require pitch deck
      default:
        return true; // Allow other steps
    }
  };
            ...
            {existingPitch?.status === 'REJECTED' && (
              <button
                onClick={() => {
                  ...
                  setCurrentStep(0);
                  setFormData(createInitialPitchData());
                  setDocuments(createInitialDocumentState());
                  setDocumentUrls(createInitialDocumentUrlState());
                  setError('');
                }}
