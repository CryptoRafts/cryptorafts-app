...
export default function FounderKYCForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        const kycDocRef = doc(db!, 'kyc_documents', user.uid);
        await setDoc(kycDocRef, {
          userId: user.uid,
          userEmail: user.email || 'N/A',
          userName: user.displayName || formData.kyc_legal_name || 'Unknown',
          status: 'pending',
          submittedAt: serverTimestamp(),
          personalInfo: {
            firstName: formData.kyc_legal_name?.split(' ')[0] || '',
            lastName: formData.kyc_legal_name?.split(' ').slice(1).join(' ') || '',
            dateOfBirth: formData.kyc_dob || '',
            nationality: formData.kyc_country || '',
            address: '',
        ...
        console.log('✅ KYC document synced to kyc_documents collection for admin visibility');
      } catch (syncError) {
        console.error('⚠️ Error syncing to kyc_documents:', syncError);
      ...
      await updateDoc(doc(db!, 'users', user.uid, 'kyc', 'verification'), {
        ...
        // IMPORTANT: Also update kyc_documents collection with verification result
        try {
          const { doc: docFn, updateDoc: updateDocFn, getDoc } = await import('firebase/firestore');
          const kycDocRef = docFn(db, 'kyc_documents', user.uid);
          ...
          if (existingDoc.exists()) {
            await updateDocFn(kycDocRef, {
              ...
              rejectionReason: verificationResult.reasons?.join(', ') || null,
              status: verificationResult.decision === 'approved' ? 'approved' : (verificationResult.decision === 'rejected' ? 'rejected' : 'pending'),
              updatedAt: serverTimestamp()
            });
          } else {
            // Document doesn't exist, create it
            const { setDoc: setDocFn2 } = await import('firebase/firestore');
            await setDocFn2(kycDocRef, {
              userId: user.uid,
              userEmail: user.email || 'N/A',
              userName: user.displayName || formData.kyc_legal_name || 'Unknown',
              status: verificationResult.decision === 'approved' ? 'approved' : (verificationResult.decision === 'rejected' ? 'rejected' : 'pending'),
          ...
          console.log('✅ KYC verification result synced to kyc_documents collection');
        } catch (syncError) {
          console.error('⚠️ Error syncing verification result to kyc_documents:', syncError);
        }
