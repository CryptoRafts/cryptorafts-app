...
export default function ReminderModal({ roomId, members, memberNames, onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const dueDateTimeString = `${dueDate}T${dueTime || '12:00'}`;
      const dueDateTimestamp = new Date(dueDateTimeString).getTime();

      await enhancedChatService.addReminder(roomId, {
        title,
        dueDate: dueDateTimestamp,
        assignedTo: assignedTo.length > 0 ? assignedTo : members,
        createdBy: members[0], // Current user
        completed: false
      });
