...
export default function InfluencerProjectDetailPage() {
  ...
  useEffect(() => {
    if (project && autoAction === 'accept' && !actionLoading) {
      handleAccept();
    } else if (project && autoAction === 'reject' && !actionLoading) {
      handleReject();
    }
  }, [project, autoAction, actionLoading]);
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/influencer/accept-campaign`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        const errorMessage = errorData.details 
          ...
          : errorData.error || errorData.message || 'Failed to accept campaign. Please try again.';
        console.error('❌ [INFLUENCER] Accept campaign error:', errorData);
        alert(errorMessage);
      }
    } catch (error: any) {
      console.error('❌ [INFLUENCER] Error accepting campaign:', error);
      const errorMessage = error?.message || 'Failed to accept campaign. Please try again.';
  ...
  const handleReject = async () => {
    ...
    try {
      const token = await user.getIdToken();
      const response = await fetch(`/api/influencer/reject-campaign`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (response.ok) {
        ...
        router.push('/influencer/dealflow');
      } else {
        const errorData = await response.json().catch(() => ({}));
        alert(errorData.error || errorData.message || 'Failed to reject campaign. Please try again.');
      }
    } catch (error) {
      ...
      <ErrorBoundary>
        <div className="min-h-screen bg-black pt-24 pb-12 px-4 flex items-center justify-center">
          <div className="text-center">
            ...
            <Link href="/influencer/dealflow" className="px-6 py-3 bg-pink-600 hover:bg-pink-700 text-white rounded-lg">
              Back to Dealflow
            </Link>
          </div>
        </div>
    ...
    <ErrorBoundary>
      ...
      >
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {/* Header */}
          <div className="mb-6">
            <Link href="/influencer/dealflow" className="inline-flex items-center text-cyan-400 hover:text-cyan-300 mb-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <>
                  <div className="bg-black/60 backdrop-blur-lg rounded-xl p-6 border-2 border-cyan-400/20">
                    ...
                    <div className="space-y-4">
                      ...
                      {project.chain && !project.blockchain && (
                        <div>
                          <label className="text-white/60 text-sm font-medium">Chain</label>
                          <p className="text-white mt-1">{project.chain}</p>
                        </div>
                      )}
                      {project.sector && (
                        <div>
                          <label className="text-white/60 text-sm font-medium">Sector</label>
                          <p className="text-white mt-1">{project.sector}</p>
                        </div>
                      )}
                      {project.geography && (
                        <div>
                          <label className="text-white/60 text-sm font-medium">Geography</label>
                          <p className="text-white mt-1">{project.geography}</p>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-black/60 backdrop-blur-lg rounded-xl p-6 border-2 border-cyan-400/20">
                ...
                {project.influencerAction !== 'accepted' && (
                  <>
                    <button
                    ...
                    >
                      {actionLoading ? 'Processing...' : 'Accept Campaign'}
                    </button>
                    <button
                ...
                {project.influencerAction === 'accepted' && (
                  <button
                    onClick={() => router.push(`/influencer/messages?room=deal_${project.founderId}_${user.uid}_${projectId}`)}
