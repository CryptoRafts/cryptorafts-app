...
export default function InfluencerMessagesPage() {
  ...
  useEffect(() => {
    ...
    console.log('📱 [INFLUENCER MESSAGES] Loading chat rooms for user:', user.uid);
    ...
    (async () => {
      try {
        const unsubscribe = await enhancedChatService.subscribeToUserRooms(
          ...
          (updatedRooms) => {
            if (!isMounted) return;
            console.log('📱 [INFLUENCER MESSAGES] Received', updatedRooms.length, 'chat rooms');
            ...
            if (roomIdFromUrl) {
              ...
              if (roomFromUrl) {
                console.log('📱 [INFLUENCER MESSAGES] Auto-selecting room from URL:', roomIdFromUrl);
    ...
    return () => {
      isMounted = false;
      console.log('📱 [INFLUENCER MESSAGES] Unsubscribing from chat rooms');
      if (unsubscribeFn) {
        unsubscribeFn();
      }
    };
  ...
  useEffect(() => {
    ...
    if (roomFromUrl && roomFromUrl.id !== selectedRoom?.id) {
      console.log('📱 [INFLUENCER MESSAGES] Selecting room from URL change:', roomIdFromUrl);
      setTimeout(() => {
        setSelectedRoom(roomFromUrl);
      }, 0);
    }
  ...
  const handleBackToList = () => {
    ...
    router.push('/influencer/messages');
  };
      ...
      <div className={`${
        ...
        <ChatRoomListProduction
          ...
          onRoomSelect={(room) => {
            ...
            router.push(`/influencer/messages?room=${room.id}`);
          }}
