...
export default function ExchangeMessagesPage() {
  ...
  useEffect(() => {
    ...
    console.log('📱 [EXCHANGE MESSAGES] Loading chat rooms for user:', user.uid);
    ...
    (async () => {
      try {
        const unsubscribe = await enhancedChatService.subscribeToUserRooms(
          ...
          (updatedRooms) => {
            if (!isMounted) return;
            console.log('📱 [EXCHANGE MESSAGES] Received', updatedRooms.length, 'chat rooms');
            ...
            if (roomIdFromUrl) {
              ...
              if (roomFromUrl) {
                console.log('📱 [EXCHANGE MESSAGES] Auto-selecting room from URL:', roomIdFromUrl);
    ...
    return () => {
      isMounted = false;
      console.log('📱 [EXCHANGE MESSAGES] Unsubscribing from chat rooms');
      if (unsubscribeFn) {
        unsubscribeFn();
      }
    };
  ...
  useEffect(() => {
    ...
    if (roomFromUrl && roomFromUrl.id !== selectedRoom?.id) {
      console.log('📱 [EXCHANGE MESSAGES] Selecting room from URL change:', roomIdFromUrl);
      setTimeout(() => {
        setSelectedRoom(roomFromUrl);
      }, 0);
    }
    ...
    <div className="flex min-h-[calc(100vh-112px)] bg-black pt-28" style={{ marginTop: 0 }}>
      <div className={`${
        ...
        <ChatRoomListProduction
          rooms={rooms}
          selectedRoomId={selectedRoom?.id || undefined}
          onRoomSelect={(room) => {
            setSelectedRoom(room);
            router.push(`/exchange/messages?room=${room.id}`);
          }}
