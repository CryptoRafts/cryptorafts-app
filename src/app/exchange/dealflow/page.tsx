...
// PolygonIcon removed - using NeonCyanIcon instead
...
export default function ExchangeDealflowPage() {
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      try {
        ...
        console.log('🔴 [EXCHANGE-DEALFLOW] Setting up REAL-TIME project listener');

        // Fetch all projects - filter client-side for flexibility
        // This ensures we catch all projects that might be seeking listing through various fields
        ...
        const unsubscribe = onSnapshot(projectsQuery, (snapshot) => {
          ...
          // CRITICAL: Match VC role filtering logic with fallback
          // Show ALL approved projects that are discoverable OR approved without visibility set
          ...
          projectsData = projectsData.filter(p => {
            ...
            const reviewStatus = (p.reviewStatus || '').toLowerCase();
              ...
              if (p.visibility) {
                if (typeof p.visibility === 'object' && p.visibility !== null) {
                  isDiscoverable = p.visibility.discoverable === true;
                } else {
                  // If visibility is a string, treat as discoverable if not explicitly hidden
                  isDiscoverable = (p.visibility as string).toLowerCase() !== 'hidden';
                }
              } else {
                // If visibility is not set, assume discoverable for approved projects
                isDiscoverable = true;
              }
              
              // EXCLUDE if this exchange already accepted it (check exchangeAction field)
              if (p.exchangeAction === 'accepted' && p.exchangeActionBy === user.uid) {
                return false; // Should be in listings/pipeline, not dealflow
              }
              
              // Show if approved AND (discoverable OR no visibility set)
              return isApproved && isDiscoverable;
            } catch (error) {
              console.error('Error filtering project:', error, p);
              return false;
            }
          });
          
          console.log(`✅ [EXCHANGE-DEALFLOW] Filtered ${projectsData.length} projects from ${beforeFilter} total (approved + discoverable)`);
          ...
          console.log('🔴 [EXCHANGE-DEALFLOW] Real-time update:', projectsData.length, 'filtered projects from', allProjectsRaw.length, 'total');
          
          // Set filtered projects for display (only approved + discoverable)
          setProjects(projectsData);
          setLoading(false);
        }, createSnapshotErrorHandler('exchange dealflow projects'));
  ...
  // Calculate stats from ALL projects (not just filtered ones) to show real numbers
  const approvedProjects = allProjects.filter((p: Project) => {
    const status = (p.status || '').toLowerCase();
    const reviewStatus = (p.reviewStatus || '').toLowerCase();
    return status === 'approved' || reviewStatus === 'approved';
  });
  
  const pendingProjects = allProjects.filter((p: Project) => {
    const status = (p.status || '').toLowerCase();
    const reviewStatus = (p.reviewStatus || '').toLowerCase();
    return (status === 'pending' || reviewStatus === 'pending' || p.adminStatus === 'pending') && 
           !approvedProjects.some((ap: Project) => ap.id === p.id);
  });
  
  const rejectedProjects = allProjects.filter((p: Project) => {
    const status = (p.status || '').toLowerCase();
    const reviewStatus = (p.reviewStatus || '').toLowerCase();
    return status === 'rejected' || reviewStatus === 'rejected' || p.adminStatus === 'rejected';
  });
            ...
            {projects.length === 0 ? (
                <>
                  ...
                  <p className="text-cyan-400/70 text-sm mb-4">
                    Projects will appear here once they are approved by admin and seeking listing
                  </p>
                </>
              ) : (
                <>
                  ...
                  <button
                    onClick={() => {
                      setSearchQuery('');
                      setFilters({ stage: [], sector: [], chain: [], geography: [], doxxed: null, audit: null, raftaiRating: [] });
                    }}
              ...
              {filteredProjects.map((project) => (
                  ...
                  <div className="flex items-center justify-between text-xs text-cyan-400/70 mb-4 pt-3 border-t border-cyan-400/20">
                    <span>
                      {project.createdAt 
                        ? (() => {
                            const timestamp = project.createdAt.toMillis 
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/exchange/project/${project.id}?action=accept`);
                      }}
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/exchange/messages?projectId=${project.id}`);
                      }}
                      className="px-3 py-2 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white rounded-lg transition-all flex items-center justify-center gap-2 text-sm font-medium shadow-lg shadow-cyan-500/20 border border-cyan-400/30"
                    >
                    ...
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/exchange/project/${project.id}?action=reject`);
                      }}
