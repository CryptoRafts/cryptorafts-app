...
export default function AgencyMessagesPage() {
  ...
  useEffect(() => {
    ...
    console.log('📱 [AGENCY MESSAGES] Loading chat rooms for user:', user.uid);
    ...
    (async () => {
      try {
        const unsubscribe = await enhancedChatService.subscribeToUserRooms(
          ...
          (updatedRooms) => {
            if (!isMounted) return;
            console.log('📱 [AGENCY MESSAGES] Received', updatedRooms.length, 'chat rooms');
            ...
            if (roomIdFromUrl) {
              ...
              if (roomFromUrl) {
                console.log('📱 [AGENCY MESSAGES] Auto-selecting room from URL:', roomIdFromUrl);
    ...
    return () => {
      isMounted = false;
      console.log('📱 [AGENCY MESSAGES] Unsubscribing from chat rooms');
      if (unsubscribeFn) {
        unsubscribeFn();
      }
    };
  ...
  useEffect(() => {
    ...
    if (roomFromUrl && roomFromUrl.id !== selectedRoom?.id) {
      console.log('📱 [AGENCY MESSAGES] Selecting room from URL change:', roomIdFromUrl);
      setTimeout(() => {
        setSelectedRoom(roomFromUrl);
      }, 0);
    }
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] bg-black" style={{ marginTop: '80px' }}>
      <div className={`${
        ...
        <ChatRoomListProduction
          ...
          onRoomSelect={(room) => {
            ...
            router.push(`/agency/messages?room=${room.id}`);
          }}
