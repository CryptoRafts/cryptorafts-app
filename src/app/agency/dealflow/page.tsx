...
export default function AgencyDealflowPage() {
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      try {
        ...
        console.log('🔴 [AGENCY-DEALFLOW] Setting up REAL-TIME project listener');
        ...
        const unsubscribe = onSnapshot(projectsQuery, (snapshot) => {
          ...
          projectsData = projectsData.filter(p => {
            ...
            // EXCLUDE if this agency already accepted it (check agencyAction field)
            if (p.agencyAction === 'accepted' && p.agencyActionBy === user.uid) {
            ...
            // Must be seeking services (check multiple possible flags)
            const isSeekingServices = p.seekingServices === true ||
                                    (p.targetRoles && Array.isArray(p.targetRoles) && p.targetRoles.includes('agency'));
            
            // Only show if approved AND discoverable AND seeking services AND not accepted by this user
            return isApproved && isDiscoverable && isSeekingServices;
          });
          ...
          console.log('🔴 [AGENCY-DEALFLOW] Real-time update:', projectsData.length, 'filtered projects from', allProjectsRaw.length, 'total');
          setProjects(projectsData);
          setLoading(false);
        }, createSnapshotErrorHandler('agency dealflow projects'));
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <p className="text-gray-300 mt-1 text-sm sm:text-base">
                  {projects.length} projects available for marketing services
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {filteredProjects.map((project) => (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => router.push(`/agency/project/${project.id}`)}
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/agency/project/${project.id}?action=accept`);
                      }}
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/agency/messages?projectId=${project.id}`);
                      }}
                      className="px-3 py-2 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white rounded-lg transition-all flex items-center justify-center gap-2 text-sm font-medium shadow-lg shadow-blue-500/20 border border-blue-400/30"
                    >
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/agency/project/${project.id}?action=reject`);
                      }}
