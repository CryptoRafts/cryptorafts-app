...
export default function VCMessagesPage() {
  ...
  useEffect(() => {
    ...
    console.log('📱 [VC MESSAGES] Loading chat rooms for user:', user.uid);
    ...
    (async () => {
      try {
        const unsubscribe = await enhancedChatService.subscribeToUserRooms(
          ...
          (updatedRooms) => {
            if (!isMounted) return;
            console.log('📱 [VC MESSAGES] Received', updatedRooms.length, 'chat rooms');
            ...
            if (roomIdFromUrl) {
              ...
              if (roomFromUrl) {
                console.log('📱 [VC MESSAGES] Auto-selecting room from URL:', roomIdFromUrl);
    ...
    return () => {
      isMounted = false;
      console.log('📱 [VC MESSAGES] Unsubscribing from chat rooms');
      if (unsubscribeFn) {
        unsubscribeFn();
      }
    };
  ...
  useEffect(() => {
    ...
    if (roomFromUrl && roomFromUrl.id !== selectedRoom?.id) {
      console.log('📱 [VC MESSAGES] Selecting room from URL change:', roomIdFromUrl);
      setTimeout(() => {
        setSelectedRoom(roomFromUrl);
      }, 0);
    }
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] bg-black">
      ...
      } w-full lg:w-80 flex-col border-r border-gray-800 bg-black/50`}>
        <ChatRoomListProduction
          rooms={rooms}
          selectedRoomId={selectedRoom?.id || null}
          onRoomSelect={(room) => {
            setSelectedRoom(room);
            router.push(`/vc/messages?room=${room.id}`);
          }}
