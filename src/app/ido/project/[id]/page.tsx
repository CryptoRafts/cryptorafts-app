...
export default function IDOProjectDetailPage() {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push(`/ido/messages?room=${data.chatId || data.roomId || `deal_${project.founderId}_${user.uid}_${projectId}`}`);
      } else {
        ...
        console.error('❌ [IDO] Accept pitch error:', errorData);
        alert(errorMessage);
      }
    } catch (error: any) {
      console.error('❌ [IDO] Error accepting pitch:', error);
  ...
  const handleReject = async () => {
    ...
    if (!confirm('Are you sure you want to reject this IDO application?')) return;
    
    setActionLoading(true);
    try {
      const token = await user.getIdToken();
      const response = await fetch(`/api/ido/reject-pitch`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (response.ok) {
        alert('IDO application rejected.');
        router.push('/ido/dealflow');
      } else {
        const errorData = await response.json().catch(() => ({}));
      ...
      <ErrorBoundary>
        <div className="min-h-screen bg-black pt-24 pb-12 px-4 flex items-center justify-center">
          <div className="text-center">
            ...
            <Link href="/ido/dealflow" className="px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white rounded-lg border-2 border-cyan-400/50 shadow-lg shadow-cyan-500/20">
              Back to Dealflow
            </Link>
          </div>
        </div>
    ...
    <ErrorBoundary>
      <div className="min-h-screen bg-black pt-24 pb-12 px-4">
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {/* Header */}
          <div className="mb-6">
            <Link href="/ido/dealflow" className="inline-flex items-center text-cyan-400 hover:text-cyan-300 mb-4">
          ...
          <div className="bg-black/40 backdrop-blur-lg rounded-xl mb-6 border border-white/10">
            <div className="flex overflow-x-auto">
              {[
                ...
                ...(project.idoAction === 'accepted' ? [{ id: 'milestones', label: 'Milestones', icon: 'check' }] : [])
              ].map(tab => (
                <button
                  key={tab.id}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              ...
              {/* Milestones Tab - Show if project is accepted by any IDO */}
              {activeTab === 'milestones' && project.idoAction === 'accepted' && (
                <div className="bg-black/60 backdrop-blur-lg rounded-xl p-6 border-2 border-cyan-400/20">
                  <h2 className="text-2xl font-bold text-white mb-6">Project Milestones</h2>
                  <p className="text-white/60 text-sm mb-6">Track key milestones for this IDO. Mark each step as complete when finished.</p>
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      return (
                        <div
                          key={milestone.id}
                          className={`bg-black/40 backdrop-blur-lg rounded-lg p-4 border-2 transition-all ${
                            isCompleted 
                          ...
                          <div className="flex items-start justify-between">
                            ...
                            <button
                              onClick={async () => {
                                ...
                                try {
                                  const token = await user.getIdToken();
                                  const response = await fetch('/api/ido/update-milestone', {
                                    method: 'POST',
                                    headers: {
                                      'Content-Type': 'application/json',
                                      'Authorization': `Bearer ${token}`
            ...
            <div className="space-y-6">
              {/* RaftAI Summary */}
              {raftai && (
                <div className="bg-black/60 backdrop-blur-lg rounded-xl p-6 border-2 border-cyan-500/30 bg-gradient-to-br from-cyan-500/10 to-blue-500/10">
                  <div className="flex items-center gap-2 mb-4">
                    <NeonCyanIcon type="cpu" size={24} className="text-cyan-400" />
              ...
              <div className="bg-black/60 backdrop-blur-lg rounded-xl p-6 border-2 border-cyan-400/20">
                ...
                {project.idoAction !== 'accepted' && (
                  <>
                    ...
                    >
                      {actionLoading ? 'Processing...' : 'Accept IDO Application'}
                    </button>
                    <button
                    ...
                    >
                      {actionLoading ? 'Processing...' : 'Reject IDO Application'}
                    </button>
                  </>
                )}
                {project.idoAction === 'accepted' && (
                  <button
                    onClick={() => router.push(`/ido/messages?room=deal_${project.founderId}_${user.uid}_${projectId}`)}
