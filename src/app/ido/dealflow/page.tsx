...
export default function IDODealflowPage() {
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      try {
        ...
        console.log('🔴 [IDO-DEALFLOW] Setting up REAL-TIME project listener');
        ...
        const unsubscribe = onSnapshot(projectsQuery, (snapshot) => {
          ...
          projectsData = projectsData.filter(p => {
            try {
              ...
              // EXCLUDE if this IDO already accepted it (check idoAction field)
              if (p.idoAction === 'accepted' && p.idoActionBy === user.uid) {
                return false; // Should be in launchpad/pipeline, not dealflow
              }
              ...
              return false; // Exclude projects that cause errors
            }
          });
          
          console.log(`✅ [IDO-DEALFLOW] Filtered ${projectsData.length} projects from ${beforeFilter} total (approved + discoverable)`);
          ...
          console.log('🔴 [IDO-DEALFLOW] Real-time update:', projectsData.length, 'filtered projects from', allProjectsRaw.length, 'total');
          
          // Set filtered projects for display (only approved + discoverable)
          setProjects(projectsData);
          setLoading(false);
        }, createSnapshotErrorHandler('IDO dealflow projects'));
  ...
  const pendingProjects = useMemo(() => {
    return allProjects.filter(p => {
      const status = (p.status || '').toLowerCase();
      const reviewStatus = (p.reviewStatus || '').toLowerCase();
      return (status === 'pending' || reviewStatus === 'pending') && 
             !approvedProjects.some(ap => ap.id === p.id);
    });
  }, [allProjects, approvedProjects]);
        ...
        try {
          ...
          if (searchQuery) {
            ...
            const matchesSearch = 
              ...
              (project.sector && project.sector.toLowerCase().includes(searchLower));
          ...
          console.error('Error filtering individual project:', error, project);
          return false;
        }
      });
    } catch (error) {
      console.error('Error in filteredProjects useMemo:', error);
      return [];
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <div className="neo-glass-card rounded-xl p-6">
              <div className="flex items-center justify-between">
                ...
                <NeonCyanIcon type="x" className="text-red-400" size={32} />
              </div>
            </div>
          </div>
              ...
              {projects.length === 0 ? (
                <>
                  ...
                  <p className="text-cyan-400/70 text-sm mb-4">
                    Projects will appear here once they are approved by admin and seeking IDO
                  </p>
                </>
              ) : (
                <>
              ...
              {filteredProjects.map((project: Project) => (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => router.push(`/ido/project/${project.id}`)}
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/ido/project/${project.id}?action=accept`);
                      }}
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/ido/messages?projectId=${project.id}`);
                      }}
                      className="px-3 py-2 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white rounded-lg transition-all flex items-center justify-center gap-2 text-sm font-medium shadow-lg shadow-cyan-500/20 border border-cyan-400/30"
                    >
