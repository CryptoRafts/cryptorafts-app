...
if (typeof window === 'undefined') {
  ...
  initAdmin();
    ...
    console.warn('⚠️ [EXCHANGE API] Module load initialization failed (will retry in handler):', error?.message);
  }
}
...
export async function POST(req: NextRequest){
  try{
    ...
    if (!auth) {
      console.log('🔄 [EXCHANGE API] Firebase Admin Auth not found, initializing...');
      try {
      // Try to initialize
      const { initAdmin } = await import('@/server/firebaseAdmin');
      const app = initAdmin();
      if (app) {
          console.log('✅ [EXCHANGE API] Firebase Admin app initialized');
        auth = getAdminAuth();
        }
      } catch (initError: any) {
        console.error('❌ [EXCHANGE API] Initialization error:', initError?.message || initError);
        // Don't fail yet, try other methods
      }
      
      if (!auth) {
        console.log('🔄 [EXCHANGE API] Trying explicit initialization...');
        try {
        // Try one more time with explicit initialization
        const { getAuth } = await import('firebase-admin/auth');
        const { getApps } = await import('firebase-admin/app');
        const apps = getApps();
          console.log(`📊 [EXCHANGE API] Found ${apps.length} Firebase Admin apps`);
        if (apps.length > 0) {
          auth = getAuth(apps[0]);
            console.log('✅ [EXCHANGE API] Got Auth from existing app');
          }
        } catch (explicitError: any) {
          console.error('❌ [EXCHANGE API] Explicit initialization error:', explicitError?.message || explicitError);
        }
      }
      
        if (!auth) {
        console.error('❌ [EXCHANGE API] Firebase Admin Auth not available after initialization attempts');
        const envCheck = {
        ...
        console.error('❌ [EXCHANGE API] Environment check:', envCheck);
    ...
    console.log('✅ [EXCHANGE API] Firebase Admin Auth ready');
    
    const decoded = await auth.verifyIdToken(token).catch((err) => {
      console.error('❌ [EXCHANGE API] Token verification failed:', err);
      return null;
    });
    if(!decoded) return NextResponse.json({error:"Invalid or expired token"}, {status:401});
    const uid = decoded.uid;
    console.log(`✅ [EXCHANGE API] Authenticated user: ${uid}`);

    let db = getAdminDb();
    if(!db){
      console.log('🔄 [EXCHANGE API] Firebase Admin DB not found, initializing...');
      try {
      // Try to initialize
      const { initAdmin } = await import('@/server/firebaseAdmin');
      const app = initAdmin();
      if (app) {
          console.log('✅ [EXCHANGE API] Firebase Admin app initialized for DB');
        db = getAdminDb();
        }
      } catch (initError: any) {
        console.error('❌ [EXCHANGE API] DB initialization error:', initError?.message || initError);
      }
      
      if(!db){
        console.log('🔄 [EXCHANGE API] Trying explicit DB initialization...');
        try {
        // Try one more time with explicit initialization
        ...
        if (apps.length > 0) {
          db = getFirestore(apps[0]);
            console.log('✅ [EXCHANGE API] Got Firestore from existing app');
        }
        } catch (explicitError: any) {
          console.error('❌ [EXCHANGE API] Explicit DB initialization error:', explicitError?.message || explicitError);
        }
      }
      
        if(!db){
        console.error('❌ [EXCHANGE API] Firebase Admin DB not available after initialization attempts');
        return NextResponse.json({ 
    ...
    console.log('✅ [EXCHANGE API] Firebase Admin DB ready');
    ...
    const projSnap = await projRef.get();
      ...
      console.error('❌ [EXCHANGE API] Error fetching project:', dbError);
    ...
    // Get names and logos
    let exchangeDoc, founderDoc;
    try {
      exchangeDoc = await db.collection("users").doc(uid).get();
      founderDoc = await db.collection("users").doc(proj.founderId).get();
    } catch (userError: any) {
      console.error('❌ [EXCHANGE API] Error fetching user data:', userError);
      return NextResponse.json({
        error: "Database error",
    ...
    const exchangeData = exchangeDoc.exists ? exchangeDoc.data() : {};
    const founderData = founderDoc.exists ? founderDoc.data() : {};
    
    const exchangeName = exchangeData?.displayName || exchangeData?.companyName || exchangeData?.organizationName || "Exchange Partner";
    const founderName = founderData?.displayName || founderData?.companyName || "Founder";
    const exchangeLogo = exchangeData?.photoURL || exchangeData?.logo || null;
    const founderLogo = founderData?.photoURL || founderData?.logo || null;

    // Update project status to accepted
    try {
    await projRef.update({
      exchangeAction: 'accepted',
      exchangeActionBy: uid,
      exchangeActionAt: Date.now(),
      updatedAt: Date.now()
    });
    } catch (updateError: any) {
      console.error('❌ [EXCHANGE API] Error updating project:', updateError);
      return NextResponse.json({
        error: "Database error",
    ...
    await db.collection("relations").doc(rid).set({
      ...
      console.error('❌ [EXCHANGE API] Error creating relation:', relationError);
      return NextResponse.json({
        error: "Database error",
    ...
    try {
      ...
      console.error('❌ [EXCHANGE API] Error checking chat room:', chatError);
      return NextResponse.json({
        error: "Database error",
    ...
    if (!existingChat.exists) {
      ...
      try {
      await chatRef.set({
        name: `${proj.title || proj.name || "Project"} - ${founderName} / ${exchangeName}`,
        type: "listing",
        status: "active",
        ...
        counterpartId: uid,
        counterpartName: exchangeName,
        counterpartRole: "exchange",
        counterpartLogo: exchangeLogo,
      ...
      await chatRef.collection("messages").add({
        ...
        senderAvatar: null,
        type: "system",
        text: `🎉 RaftAI created this listing room for ${founderName} and ${exchangeName}. You can now discuss the token listing, requirements, and next steps!`,
        reactions: {},
        readBy: [],
        isPinned: false,
        isEdited: false,
      ...
      console.log(`✅ [CHAT] Listing room created: ${chatId}`);
      } catch (chatCreateError: any) {
        console.error('❌ [EXCHANGE API] Error creating chat room:', chatCreateError);
        return NextResponse.json({
          error: "Database error",
    ...
    try {
      ...
      await db.collection('notifications').add({
        ...
        title: 'Exchange Accepted Your Pitch! 🎉',
        message: `${exchangeName} (Exchange) has accepted your pitch "${projectName}". You can now collaborate in the chat room.`,
        data: {
          projectId,
          projectName,
          roomId: chatId,
          acceptedBy: uid,
          acceptedByName: exchangeName,
          acceptedByRole: 'exchange',
          roomUrl: `/founder/messages?room=${chatId}`,
          founderLogo: founderLogo,
          partnerLogo: exchangeLogo
        },
      ...
      console.log(`✅ [EXCHANGE API] Notification created for founder: ${proj.founderId}`);
    } catch (notificationError: any) {
      console.warn(`⚠️ [EXCHANGE API] Failed to create notification (non-critical):`, notificationError.message);
    ...
    console.error('❌ [EXCHANGE API] Error in accept-pitch:', e);
    console.error('❌ [EXCHANGE API] Error stack:', e?.stack);
    console.error('❌ [EXCHANGE API] Error name:', e?.name);
    console.error('❌ [EXCHANGE API] Error code:', e?.code);
