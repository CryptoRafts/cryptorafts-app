...
# Deploy everything including VPS, DNS, and app configuration
# ============================================

Write-Host "`n🚀 CRYPTORAFTS - COMPLETE DEPLOYMENT" -ForegroundColor Cyan
...
Write-Host "2. Verify/update Firebase configuration" -ForegroundColor White
Write-Host "3. Verify/update .env.local with all API keys" -ForegroundColor White
Write-Host "4. Rebuild Next.js application" -ForegroundColor White
Write-Host "5. Update server.js and ecosystem.config.js" -ForegroundColor White
Write-Host "6. Restart PM2 process" -ForegroundColor White
Write-Host "7. Configure Nginx for www.cryptorafts.com" -ForegroundColor White
Write-Host "8. Verify SSL certificate" -ForegroundColor White
Write-Host "9. Verify DNS configuration" -ForegroundColor White
Write-Host "10. Final verification" -ForegroundColor White
Write-Host ""

# Complete deployment command
$deployCommand = @"
cd $appPath && echo "🚀 COMPLETE DEPLOYMENT - www.cryptorafts.com" && echo "========================================" && echo "" && 
...
if [ -f .env.local ]; then
  if grep -q 'OPENAI_API_KEY=sk-' .env.local && grep -q 'NEXT_PUBLIC_FIREBASE_API_KEY' .env.local; then
    echo "✅ .env.local exists with all API keys"
  else
...
echo "📋 Step 3: Stopping PM2 and cleaning..." && 
pm2 stop cryptorafts 2>/dev/null || echo "⚠️  PM2 not running" && 
pm2 delete cryptorafts 2>/dev/null || echo "⚠️  PM2 process not found" && 
rm -rf .next node_modules/.cache && 
echo "✅ Cleaned" && echo "" && 

# Step 4: Load Node.js 20
echo "📋 Step 4: Loading Node.js 20..." && 
...
echo "📋 Step 6: Updating server.js..." && 
...
echo "📋 Step 7: Updating ecosystem.config.js..." && 
...
if [ -f /etc/letsencrypt/live/cryptorafts.com/fullchain.pem ]; then
  ...
  openssl x509 -in /etc/letsencrypt/live/cryptorafts.com/fullchain.pem -noout -subject -dates 2>/dev/null | head -2 || echo "Certificate found"
else
  echo "⚠️  SSL certificate not found - installing..."
  certbot --nginx -d cryptorafts.com -d www.cryptorafts.com --non-interactive --agree-tos --email admin@cryptorafts.com --redirect 2>&1 | tail -10
fi
echo "" && 

# Step 11: Check DNS
echo "📋 Step 11: Checking DNS configuration..." && 
echo "Current DNS records:" && 
dig +short cryptorafts.com A && 
dig +short www.cryptorafts.com A && 
echo "" && 
echo "Expected IP: $vpsIP" && 
echo "" && 
...
echo "" && 
curl -sS -I https://www.cryptorafts.com 2>&1 | head -5 || echo "Testing www.cryptorafts.com..." && 
echo "" && 
...
echo "A record for @ (cryptorafts.com) should point to: $vpsIP"
echo "A record for www should point to: $vpsIP"
echo ""
...
Write-Host "Host: $vpsIP" -ForegroundColor White
Write-Host "User: $vpsUser`n" -ForegroundColor White

Write-Host "⚠️  NOTE: You'll need to enter your SSH password when prompted" -ForegroundColor Yellow
Write-Host "⚠️  The deployment will run automatically after authentication`n" -ForegroundColor Yellow
