# ============================================
# FRESH BUILD AND DEPLOY TO VPS
# Complete fresh build and deployment
# ============================================
...
$sshCommand = "pm2 stop cryptorafts 2>/dev/null || true; pm2 delete cryptorafts 2>/dev/null || true; rm -rf $vpsPath; mkdir -p $vpsPath; echo '✅ VPS cleared'"
sshpass -p $vpsPassword ssh -o StrictHostKeyChecking=no -o ConnectTimeout=30 -o ServerAliveInterval=60 "${vpsUser}@${vpsIp}" $sshCommand
if ($LASTEXITCODE -eq 0) {
...
foreach ($file in $configFiles) {
    if (Test-Path $file) {
        sshpass -p $vpsPassword scp -o StrictHostKeyChecking=no -o ConnectTimeout=30 -o ServerAliveInterval=60 $file "${vpsUser}@${vpsIp}:${vpsPath}/$file" | Out-Null
        if ($LASTEXITCODE -eq 0) {
...
if (Test-Path "src") {
    sshpass -p $vpsPassword scp -r -o StrictHostKeyChecking=no -o ConnectTimeout=30 -o ServerAliveInterval=60 "src" "${vpsUser}@${vpsIp}:${vpsPath}/" | Out-Null
    if ($LASTEXITCODE -eq 0) {
...
if (Test-Path "public") {
    sshpass -p $vpsPassword scp -r -o StrictHostKeyChecking=no -o ConnectTimeout=30 -o ServerAliveInterval=60 "public" "${vpsUser}@${vpsIp}:${vpsPath}/" | Out-Null
    if ($LASTEXITCODE -eq 0) {
...
$buildCommand = "cd $vpsPath && npm install --legacy-peer-deps && npm run build && echo '✅ Build complete'"
sshpass -p $vpsPassword ssh -o StrictHostKeyChecking=no -o ConnectTimeout=30 -o ServerAliveInterval=60 "${vpsUser}@${vpsIp}" $buildCommand
if ($LASTEXITCODE -eq 0) {
...
$startCommand = "cd $vpsPath && pm2 start npm --name cryptorafts -- start && pm2 save && pm2 startup && echo '✅ PM2 started'"
sshpass -p $vpsPassword ssh -o StrictHostKeyChecking=no -o ConnectTimeout=30 -o ServerAliveInterval=60 "${vpsUser}@${vpsIp}" $startCommand
if ($LASTEXITCODE -eq 0) {
