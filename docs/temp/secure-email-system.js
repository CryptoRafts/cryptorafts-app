...
/**
 * 🔒 SECURE EMAIL SYSTEM - Password Protected
 * This keeps your password secret and sets up business@cryptorafts.com
 */

import nodemailer from 'nodemailer';
import { readFileSync, existsSync } from 'fs';

console.log('🔒 CryptoRafts Secure Email System');
console.log('==================================\n');

// Secure email service with environment variables
class SecureEmailService {
  ...
  loadSecureConfig() {
    ...
    if (existsSync('.env.local')) {
      try {
        ...
        return {
          ...
          auth: {
            user: envVars.EMAIL_USER || 'business@cryptorafts.com',
            pass: envVars.EMAIL_PASSWORD || 'YOUR_SECURE_PASSWORD_HERE'
          }
        };
      } catch (error) {
        console.log('⚠️ Could not read .env.local, using default config');
      }
    }
    ...
    return {
      ...
      auth: {
        ...
        pass: 'YOUR_SECURE_PASSWORD_HERE' // This will be replaced
      }
    };
  }
  ...
  async sendEmail(userData) {
    try {
      ...
      console.log('✅ Secure Gmail connection successful!');
  ...
  async testSecureSystem() {
    console.log('🧪 Testing Secure Email System...\n');
    ...
    if (result) {
      console.log('\n🎉 SUCCESS! Secure email system working!');
      console.log('📧 Professional CryptoRafts email sent!');
      console.log('\n🔒 Your password is secure and protected!');
    } else {
      console.log('\n❌ Setup needed:');
      console.log('1. Create .env.local file with your secure password');
      console.log('2. Set up business@cryptorafts.com in Gmail');
