...
# IT WILL FIND THE FILES AUTOMATICALLY
# ============================================
...
# Step 1: Find the deployment script
Write-Host "Step 1: Finding deployment files..." -ForegroundColor Yellow

# Search for the script in common locations
$scriptPath = $null
$searchPaths = @(
    "$PWD\$scriptFile",
    "$env:USERPROFILE\cryptorafts-starter\$scriptFile",
    "$env:USERPROFILE\Documents\cryptorafts-starter\$scriptFile",
    "$env:USERPROFILE\Desktop\cryptorafts-starter\$scriptFile",
    (Get-ChildItem -Path $env:USERPROFILE -Recurse -Filter $scriptFile -ErrorAction SilentlyContinue | Select-Object -First 1).FullName
)
...
foreach ($path in $searchPaths) {
    if ($path -and (Test-Path $path)) {
        ...
        Write-Host "SUCCESS: Found $scriptFile in: $scriptDir" -ForegroundColor Green
        Write-Host "Changed to directory: $scriptDir" -ForegroundColor Cyan
        break
    }
}
...
if (-not $scriptPath) {
    ...
    Write-Host "  cd C:\Users\dell\cryptorafts-starter" -ForegroundColor Cyan
    Write-Host ""
